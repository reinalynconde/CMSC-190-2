...
namespace SFM {
  ...
  bool notify_node(string uname) {
    sio::client h;
    socket::ptr current_socket;

    connection_listener l = connection_listener(h);
    ...
    // h.socket()->emit("cppmessage", uname);

    current_socket->emit("cppmessage", uname);
    h.sync_close();
  ...
  bool SfmData::set_dir(const string input_dir, const string output_d,
    string user) {
    ...
    for(vector<string>::const_iterator i = imags.begin();
      ...
      string str_p = to_string(percent) + "";
      ...
      cout << percent << endl;
      string sql_query = "UPDATE users set Progress = '" + progress
        + "', Percent = '" + str_p + "', Step = '1' WHERE UserId = '" + uname + "';";

      cout << sql_query << endl;

      /* Execute SQL statement */
      rc = sqlite3_exec(db, sql_query.c_str(), callback, (void*)data, &zErrMsg);
      ...
      sleep(5);
      // cout << "[POKRA] " << notify_node(uname) << endl;
    }

    return true;
  } //set_dir end

  bool SfmData::reconstruct() {
    if (images.size() <= 0) {
...
//AAAAAAAAAAAAAAAAOBJFILE
  ...
  void SfmData::detect_features() {
    ...
    for(size_t i = 0; i < i_size; i++) {
      ...
      cout << percent << endl;
      string sql_query = "UPDATE users set Progress = '" + progress
        + "', Percent = '" + str_p + "', Step = '2' WHERE UserId = '" + uname + "';";
  ...
  bool SfmData::feature_match() {
    ...
    for(size_t i = 0; i < (i_size - 1); i++) {
      for(size_t j = (i + 1); j < i_size; j++) {
        ...
        progress = string("Matching") + to_string((int)i) + string(" and ") + to_string((int)j);
        percent = c / total * 100;
        string str_p = to_string(percent) + "";
      ...
      cout << percent << endl;
      string sql_query = "UPDATE users set Progress = '" + progress
        + "', Percent = '" + str_p + "', Step = '3' WHERE UserId = '" + uname + "';";
  ...
  void SfmData::select_initial_pair() {
    ...
    map<float, Pair> h_inliers = rank_matches();
  ...
  void SfmData::add_camera() {
    ...
    while (done_v.size() != images.size()) {
      ...
      cout << percent << endl;
      string sql_query = "UPDATE users set Progress = '" + progress
        + "', Percent = '" + str_p + "', Step = '4' WHERE UserId = '" + uname + "';";
  ...
  static void WorkAsync(uv_work_t *req) {
    ...
    string inputDir = work->inputDir;
    string outputDir = work->outputDir;
    string user = work->user;

    if(sfm.set_dir(inputDir, outputDir, user))
      // cout << "HEEEY" << endl;
      sfm.reconstruct();
    ...
    /* Create merged SQL statement */
    string sql_query = "DELETE FROM users WHERE UserId = '" + sfm.uname + "';";
  ...
  void commence(const FunctionCallbackInfo<Value>& args) {
    ...
    string user(* v8::String::Utf8Value(args[2]->ToString()));

    work->inputDir = inputDir;
    work->outputDir = outputDir;
    work->user = user;
...
/*  int participants = -1;
  ...
  void init(Local<Object> exports) {
    NODE_SET_METHOD(exports, "commence", commence);
    return;
  }
