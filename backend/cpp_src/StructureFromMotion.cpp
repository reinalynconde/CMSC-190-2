...
namespace SFM {
  ...
  bool SfmData::set_dir(const string input_dir, const string output_d,
    ...
    for(vector<string>::const_iterator i = imags.begin();
      ...
      progress = to_string((int)images.size())
        + string("/") + to_string((int)imags.size());
      ...
      cout << progress << endl;
      string sql_query = "UPDATE users set Progress = '" +
        progress + "' where UserId = '" + uname + "';";
...
//AAAAAAAAAAAAAAAAOBJFILE
  ...
  void SfmData::detect_features() {
    ...
    for(size_t i = 0; i < i_size; i++) {
      ...
      progress = to_string((int)i) + string("/") + to_string((int)i_size);
  ...
  void SfmData::add_camera() {
    ...
    while (done_v.size() != images.size()) {
      ...
      progress = to_string((int)done_v.size())
        + string("/") + to_string((int)images.size());
