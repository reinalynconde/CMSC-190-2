...
bool Save(ArchiveSave& a, const custom_cv::Matx<_Tp,m,n>& _m) {
	a.stream.write((const char*)_m.val, sizeof(_Tp)*m*n);
	return true;
}
template<typename _Tp, int m, int n>
bool Load(ArchiveLoad& a, custom_cv::Matx<_Tp,m,n>& _m) {
...
template<typename _Tp>
bool Save(ArchiveSave& a, const custom_cv::Point3_<_Tp>& pt) {
	a.stream.write((const char*)&pt.x, sizeof(_Tp)*3);
	return true;
}
template<typename _Tp>
bool Load(ArchiveLoad& a, custom_cv::Point3_<_Tp>& pt) {
...
struct Interface
{
	typedef custom_cv::Point3_<float> Pos3f;
	typedef custom_cv::Point3_<double> Pos3d;
	typedef custom_cv::Matx<double,3,3> Mat33d;
	typedef custom_cv::Matx<double,4,4> Mat44d;
	typedef custom_cv::Point3_<uint8_t> Col3; // x=B, y=G, z=R
	/*----------------------------------------------------------------*/
